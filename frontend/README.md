...
NEXT_PUBLIC_API_URL=https://apiatechskills.vercel.app/api/v1
...
The login and register pages include **Continue with Google**. The button redirects to the backend at:

```txt
https://apiatechskills.vercel.app/api/v1/auth/google
```

After Google approves the login, the backend returns to:

```txt
https://atechskills.com/auth/google/callback
```

That callback stores the AtechSkills JWT and sends the student to `/student-dashboard`.
