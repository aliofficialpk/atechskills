...
adminRouter.get("/courses", asyncRoute(async (_req, res) => {
...
adminRouter.get("/teachers", asyncRoute(async (_req, res) => {
...
adminRouter.post("/courses", validate(courseSchema), asyncRoute(async (req, res) => {
  ...
  await audit(req.user?.id, "COURSE_CREATED", "Course", course.id);
  res.status(201).json(course);
}));

adminRouter.patch("/courses/:id/publish", asyncRoute(async (req, res) => {
  const course = await prisma.course.update({ where: { id: String(req.params.id) }, data: { status: "PUBLISHED" } });
  await audit(req.user?.id, "COURSE_PUBLISHED", "Course", course.id);
  res.json(course);
}));

adminRouter.patch("/courses/:id/assign-teacher", asyncRoute(async (req, res) => {
  const teacher = await prisma.teacher.findUnique({ where: { id: String(req.body.teacherId ?? "") }, include: { user: true } });
  if (!teacher) return res.status(404).json({ error: "Teacher not found" });
  const course = await prisma.course.update({ where: { id: String(req.params.id) }, data: { instructorId: teacher.id }, include: { instructor: { include: { user: true } } } });
  await audit(req.user?.id, "COURSE_TEACHER_ASSIGNED", "Course", course.id);
  res.json(course);
}));

adminRouter.delete("/courses/:id", asyncRoute(async (req, res) => {
  const course = await prisma.course.update({ where: { id: String(req.params.id) }, data: { status: "ARCHIVED" } });
  await audit(req.user?.id, "COURSE_ARCHIVED", "Course", course.id);
  res.json(course);
}));
